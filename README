...
dir=/var/www/drupal/sites/all/themes/cairntheme
user=yourusername
group=yourgroup
...
echo -n "Enter the replacement colour code for gold: "
read gold

echo -n "Enter the replacement colour code for brown: "
read brown

echo -n "Enter the replacement colour code for green: "
read green
...
mv $dir/$sub2/starterkit.info.remove $dir/$sub2/$sub2.info
...
sed -i "s/f7e59d/$gold/g" $dir/$sub2/css/$sub2.css
sed -i "s/292c33/$brown/g" $dir/$sub2/css/$sub2.css
sed -i "s/517801/$green/g" $dir/$sub2/css/$sub2.css
</pre>

h3. Create header and navbar images

In the cairntheme/images-source directory, there are 2 psd files that make up the header images

 * open header.psd 
 ** fill the layer called 'fill this layer' with the gold replacement color 
 ** choose file, save for web, choose png & save the image as header.png
 * open navbar.psd
 ** fill the layer called 'fill this layer' with the green replacement color 
 ** choose file, save for web, choose png & save the image as navbar-left.png
 * open navbar.psd
 ** fill the layer called 'fill this layer' with the brown replacement color 
 ** choose file, save for web, choose png & save the image as navbar-right.png
 * open navbar.psd
 ** fill the layer called 'fill this layer' with the gold replacement color 
 ** choose file, save for web, choose png & save the image as nv-hv.png

save these images in the respective sub-sub-theme/images folder
